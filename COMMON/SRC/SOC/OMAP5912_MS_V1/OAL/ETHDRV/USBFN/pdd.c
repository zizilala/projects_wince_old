...
{
    OMAP5912_USBD_REGS *pUSBDRegs = pPdd->pUSBDRegs;
    WORD data[4];
...
//  On OMAP5912 we simply disable all end points.
//
...
//
//  This function is called when pipe to endpoit is closed. For OMAP5912 we
...
//
//  This function is called when pipe to endpoint is created. For OMAP5912
...
//  to device. For OMAP5912 this is managed by hardware.
//
...
//  UfnPdd_InitEndpoint. For OMAP5912 it isn't a case, so we should do all
...
//
//  For OMAP5912 we should check if total descriptor size is smaller
...
DWORD WINAPI UfnPdd_Init(
    ...
    pUSBDRegs = (OMAP5912_USBD_REGS*)OALPAtoUA( OMAP5912_USBD_REGS_PA );
