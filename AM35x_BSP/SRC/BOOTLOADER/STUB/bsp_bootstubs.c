...
*/
		...
		case OMAP_DEVICE_MCBSP1:
            ...
            break;
//nmcca: need support for MCBSPI2 for CAMERA 
			case OMAP_DEVICE_MCBSP2:
            pCmRegs = OALPAtoUA(OMAP_PRCM_CORE_CM_REGS_PA);
            if (bEnable)
                {                
                SETREG32(&pCmRegs->CM_FCLKEN_xxx, CM_CLKEN_MCBSP2);
                SETREG32(&pCmRegs->CM_ICLKEN_xxx, CM_CLKEN_MCBSP2);
                while (INREG32(&pCmRegs->CM_IDLEST1_xxx) & CM_IDLEST_ST_MCBSP2);
                }
            else
                {
                CLRREG32(&pCmRegs->CM_FCLKEN_xxx, CM_CLKEN_MCBSP2);
                CLRREG32(&pCmRegs->CM_ICLKEN_xxx, CM_CLKEN_MCBSP2);
                }
            break;
        ...
        case OMAP_DEVICE_HSOTGUSB:
			{
				DWORD dwReg = 0;
				OMAP_SYSC_GENERAL_REGS* pSysConfRegs = OALPAtoUA(OMAP_SYSC_GENERAL_REGS_PA);
				pCmRegs = OALPAtoUA(OMAP_PRCM_CORE_CM_REGS_PA);
			
				if (bEnable)
				{
					// Enable SCM clock
					SETREG32(&pCmRegs->CM_ICLKEN1_xxx, CM_CLKEN_SCMCTRL);
					while (INREG32(&pCmRegs->CM_IDLEST1_xxx) & CM_IDLEST_ST_SCMCTRL);

					// Enable IPSS clock
					SETREG32(&pCmRegs->CM_ICLKEN1_xxx, CM_CLKEN_IPSS);
					while (INREG32(&pCmRegs->CM_IDLEST1_xxx) & CM_IDLEST_ST_IPSS);

					dwReg = INREG32(&pSysConfRegs->CONTROL_IPSS_CLK_CTRL);
					dwReg |= (USB20OTG_VBUSP_CLK_EN | USB20OTG_FUNC_CLK_EN);
					OUTREG32(&pSysConfRegs->CONTROL_IPSS_CLK_CTRL, dwReg);
					dwReg = INREG32(&pSysConfRegs->CONTROL_IPSS_CLK_CTRL);
				}
				else
				{
					...
					// Disable IPSS clock
					CLRREG32(&pCmRegs->CM_ICLKEN1_xxx, CM_CLKEN_IPSS);

					// Disable SCM clock
					CLRREG32(&pCmRegs->CM_ICLKEN1_xxx, CM_CLKEN_SCMCTRL);
				}
			 }
            break;
/*
