...
//  SYSINTR. There is custom implementation for OMAP5912 because hardware
//  supports 127 different IRQs compared to maximal 64 IRQs allowed in generic
...
//
static UINT32 g_oalSysIntr2Irq[SYSINTR_MAXIMUM][OMAP5912_IRQ_PER_SYSINTR];
static UINT32 g_oalIrq2SysIntr[OMAP5912_IRQ_MAXIMUM];
    ...
    )
{
    ...
    for (i = 0; i < SYSINTR_MAXIMUM; i++)
        {
        for (j = 0; j < OMAP5912_IRQ_PER_SYSINTR; j++)
            {
            g_oalSysIntr2Irq[i][j] = OAL_INTR_IRQ_UNDEFINED;
            }
        }
    for (i = 0; i < OMAP5912_IRQ_MAXIMUM; i++)
        {
    ...
    )
{
    ...
    if ((irq >= OMAP5912_IRQ_MAXIMUM) || (sysIntr >= SYSINTR_MAXIMUM))
        goto cleanUp;

    for (i = 0; i < OMAP5912_IRQ_PER_SYSINTR; i++)
        {
    ...
    )
{
    ...
    *pCount = OMAP5912_IRQ_PER_SYSINTR;
    ...
    )
{
    ...
    if (irq < OMAP5912_IRQ_MAXIMUM) sysIntr = g_oalIrq2SysIntr[irq];
    ...
    )
{
    ...
    if ((flags & OAL_INTR_STATIC) != 0)
        ...
        for (i = 0; i < count; i++)
            {
            if (pIrqs[i] >= OMAP5912_IRQ_MAXIMUM)
                {
                OALMSG(OAL_ERROR, (L"ERROR: OALIntrRequestSysIntr: "
    ...
    if (pIrqs[0] >= OMAP5912_IRQ_MAXIMUM)
        {
        OALMSG(OAL_ERROR, (L"ERROR: OALIntrRequestSysIntr: "
            L"IRQ %d out of range\r\n", pIrqs[0]
            ));
    ...
    while (i < OMAP5912_IRQ_PER_SYSINTR)
        {
    ...
    )
{
    ...
    // Enable interrupts
    rc = OALIntrEnableIrqs(OMAP5912_IRQ_PER_SYSINTR, g_oalSysIntr2Irq[sysIntr]);
    ...
    )
{
    ...
    // Disable interrupts
    OALIntrDisableIrqs(OMAP5912_IRQ_PER_SYSINTR, g_oalSysIntr2Irq[sysIntr]);
    ...
    )
{
    ...
    if (mask)
        ...
        {
        OALIntrEnableIrqs(OMAP5912_IRQ_PER_SYSINTR, g_oalSysIntr2Irq[sysIntr]);
        }
    ...
    )
{
    ...
    OALIntrDoneIrqs(OMAP5912_IRQ_PER_SYSINTR, g_oalSysIntr2Irq[sysIntr]); 
