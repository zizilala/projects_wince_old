    ...
    while (!pDevice->bIntrThreadExit)
    {
        switch (pDevice->PwrkeyIntrThreadState)
        {
            case WAIT_FOR_PWRKEY_IDLE:
                DEBUGMSG(ZONE_IST, (L"+PKD_IntrThread WAIT_FOR_PWRKEY_IDLE\r\n"));
                CurrentDebounceTimeMs = pDevice->DebounceTimeMs;
                break;

            case WAIT_FOR_PWRKEY_DEBOUNCE:
                DEBUGMSG(ZONE_IST, (L"+PKD_IntrThread WAIT_FOR_PWRKEY_DEBOUNCE\r\n"));
                CurrentDebounceTimeMs = pDevice->DebounceTimeMs;
                break;

            case WAIT_FOR_PWRKEY_INTERRUPT:
                DEBUGMSG(ZONE_IST, (L"+PKD_IntrThread WAIT_FOR_PWRKEY_INTERRUPT\r\n"));
        ...
        if (pDevice->bWakeFromSuspend)
        {
            ...
            Sleep(500);
            DEBUGMSG(ZONE_IST, (L"+PKD_IntrThread resume detected\r\n"));
            continue;
        }
        
        DEBUGMSG(ZONE_IST, (L"+PKD_IntrThread %s\r\n", WaitResult == WAIT_OBJECT_0 ? L"interrupt" : L"timeout"));
