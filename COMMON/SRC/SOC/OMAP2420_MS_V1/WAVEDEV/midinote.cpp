...
HRESULT
CMidiNote::NoteOn(CMidiStream *pMidiStream, UINT32 Note, UINT32 Velocity, UINT32 Channel)
{
    // Save params
...
HRESULT
CMidiNote::NoteOff(UINT32 Velocity)
{
    ...
    DEBUGMSG(ZONE_MIDI, (TEXT("CMidiNote::NoteOff, m_Index = 0x%x, m_IndexDelta = 0x%x, m_dwBytesLeft = %d\r\n"),m_Index,m_IndexDelta,m_dwBytesLeft));
...
//------------------------------------------------------------------------------
//
//  Function: CMidiNote::Render
//
//  note interpreter
//

PBYTE
CMidiNote::Render(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast)
{
    ...
    DEBUGMSG(ZONE_MIDI, (TEXT("CMidiNote::Render, Note in release, m_dwBytesLeft = %d\r\n"),BytesLeft));
    ...
    if (BytesLeft==0)
    {
        ...
        DEBUGMSG(ZONE_MIDI, (TEXT("CMidiNote::Render, Last index after note done = 0x%x\r\n"),m_Index));
        m_pMidiStream->NoteDone(this);
    }
...
//------------------------------------------------------------------------------
//
//  Function: CMidiNote::Render2
//
//  Inner loop of the note renderer.
//

PBYTE
CMidiNote::Render2(PBYTE pBuffer, PBYTE pBufferEnd, PBYTE pBufferLast)
{
