...
TCHAR f_sdOutputFile[MAX_PATH];         // full path of output file on SD card
...
void PrintUsage()
{
    ...
    RETAILMSG(DBGZONE_ERROR, 
        (TEXT("\t/br {bitrate}         - Specify encode bitrate in kbps\r\n")));
    RETAILMSG(DBGZONE_ERROR, 
    ...
    )
{
    ...
    if (f_outputFileOnSDCard) {
        _tcscpy(f_sdOutputFile, _T("\\Storage Card\\"));
        _tcscat(f_sdOutputFile, f_outputFile);
        hr = CreateFilters((LPCOLESTR) f_sdOutputFile);
        RETAILMSG(DBGZONE_INFO, (TEXT("%s: SD card file name is %s\r\n"), APP_NAME, f_sdOutputFile));
    }
    else {
