...
DWORD StreamContext::MapGain(DWORD StreamGain)
{
    ...
    else
    {
        ...
        dBAttenStream    = ((0xFFFF - StreamGain)    * 
            g_pHWContext->GetStreamAttenMax());
        dBAttenDevice    = ((0xFFFF - DeviceGain)    * 
            g_pHWContext->GetDeviceAttenMax());
        dBAttenSecondary = ((0xFFFF - SecondaryGain) * 
            g_pHWContext->GetSecondAttenMax());
...
HRESULT 
WaveStreamContext::Open(DeviceContext *pDeviceContext, LPWAVEOPENDESC lpWOD, DWORD dwFlags)
{
    HRESULT Result;
...
DWORD 
WaveStreamContext::GetRate(DWORD *pdwMultiplier)
{
