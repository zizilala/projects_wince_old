...
//
static BOOL x86InterpretCPUIDValue(const UINT32 & cpuIDValue);
...
#define NUMVALIDCPUIDDESCRIPTORS 53
#define MAXCPUIIDITERATIONS 6

x86CPUIDPair x86CPUIDTable[NUMVALIDCPUIDDESCRIPTORS];
...
extern "C" BOOL x86IoCtlHalGetCacheInfo(
    ...
    UINT32 CPUIDInfo[MAXCPUIIDITERATIONS * 4]; // supports MAXCPUIIDITERATIONS worth of CPUID iterations

    // this loop reads the cache info from the cpu
    UINT32 iterationNumber = 0;
    while(iterationNumber < MAXCPUIIDITERATIONS)
    {
        UINT32 CPUIDTempBuffer[4];
        ...
        UINT32 iterationOffset = iterationNumber * sizeof(UINT32);

        CPUIDInfo[iterationOffset] = CPUIDTempBuffer[0];
        CPUIDInfo[iterationOffset+1] = CPUIDTempBuffer[1];
        CPUIDInfo[iterationOffset+2] = CPUIDTempBuffer[2];
        CPUIDInfo[iterationOffset+3] = CPUIDTempBuffer[3];
        ...
        if((CPUIDInfo[iterationOffset] & 0x000000FF) == 0x01)
        {
            break;
        }
    }
    ...
    // update our global cache struct based on the values we read from the cpu
    while(iterationNumber != 0)
    {
        iterationNumber--;
        x86InterpretCPUIDValue(CPUIDInfo[iterationNumber]);
        x86InterpretCPUIDValue(CPUIDInfo[iterationNumber+1]);
        x86InterpretCPUIDValue(CPUIDInfo[iterationNumber+2]);
        x86InterpretCPUIDValue(CPUIDInfo[iterationNumber+3]);
    }
...
//
static BOOL ProcessDescriptorByte(const byte & descriptor);
...
static BOOL x86InterpretCPUIDValue(const UINT32 & cpuIDValue)
{
    ...
    return (ProcessDescriptorByte(cpuIDValue & 0xFF) &&
            ProcessDescriptorByte(cpuIDValue >> 8 & 0xFF) &&
            ProcessDescriptorByte(cpuIDValue >> 16 & 0xFF) &&
            ProcessDescriptorByte(cpuIDValue >> 24 & 0xFF));
}
...
static BOOL ProcessDescriptorByte(const BYTE & descriptor)
{
    for(int loopCounter = 0; loopCounter < NUMVALIDCPUIDDESCRIPTORS; loopCounter++)
    {
        if(descriptor == x86CPUIDTable[loopCounter].descriptor)
        {
            ...
            // because we shouldn't overlapping entries, we can simply
            // logical or the info into our global cache info
...
static void x86InitializeCPUIDTable()
{
    ...
    x86CPUIDTable[51].descriptor = 0xF0; // 64-Byte Prefetching
    x86CPUIDTable[52].descriptor = 0xF1; // 128-Byte Prefetching
}
