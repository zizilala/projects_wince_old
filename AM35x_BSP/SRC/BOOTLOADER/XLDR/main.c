...
//------------------------------------------------------------------------------
//
//  Function:  XLDRReadChar
//
//  Input char from debug serial port
//
UINT8 XLDRReadChar(VOID)
{
	INT c;
	...
	while ( (c == OEM_DEBUG_READ_NODATA) || (c == OEM_DEBUG_COM_ERROR));
...
//------------------------------------------------------------------------------
//
//  Function:  XLDRReadCharExt
//
//  Input char from debug serial port, returns error code {-1, char}
//
INT XLDRReadCharExt(UINT8 *uc)
{
	INT c;
...
//------------------------------------------------------------------------------
//
//  Function:  XLDRHasInData
//
//  
//
INT XLDRHasInChar(VOID)
{
	/*TODO*/return (0);
}
#endif /* UART_BOOT */
...
VOID XLDRMain()
{
...
#if 0
    for (dnld_size=0; dnld_size < 10000; dnld_size++) XLDRMSG(L"XLDRUART started\r\n");

	// Empty the input queue first
	nand_blocks_to_write = 0;
	image_block_cnt = 0;
	do
	{
	    XLDRPrintUint8((unsigned char)nand_blocks_to_write, 1, 1); 
		nand_blocks_to_write = OEMReadDebugByte();
		++image_block_cnt;
	} while ((nand_blocks_to_write != OEM_DEBUG_READ_NODATA) && (nand_blocks_to_write != OEM_DEBUG_COM_ERROR));
	...
	{
	uc = XLDRReadChar ();
	XLDRPrintUint8((unsigned char)uc, 1, 1);
	}
#endif
	...
	xret = XReceive(pImage, IMAGE_XLDR_BOOTSEC_NAND_SIZE + 
							IMAGE_EBOOT_CODE_SIZE		 +
	    ...
	    if(image_block_cnt == 4)
	    	{
                  ...
                  FMD_Deinit(hFMD); 
                  hFMD = FMD_Init(NULL, &regInfo, NULL);
                  XLDRMSG(L"\r\nDNLD flash EBOOT Image\r\n");
		    if(dwEbootECCtype == 1)
                      XLDRMSG(L"\r\nbch4\r\n");
		    else if(dwEbootECCtype == 2)
                      XLDRMSG(L"\r\nbch8\r\n");
		    else if(dwEbootECCtype == 0)
                      XLDRMSG(L"\r\nhamming\r\n");
		    else
                      XLDRMSG(L"\r\ndefault\r\n");
	    	}
