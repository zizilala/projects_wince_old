...
VOID HotKey(BOOL bForced)
{
    ...
    while (!bForced && (delay > 0 && key != GPIOGetBit(hGPIO, 114) && key != GPIOGetBit(hGPIO, 115) )) 
	{
		...
		while ((OALGetTickCount() - time) < 1000) 
		{
			...
			if ( (key = OALBLMenuReadKey(FALSE)) == GPIOGetBit(hGPIO, 114) && (key = OALBLMenuReadKey(FALSE)) == GPIOGetBit(hGPIO, 115) ) 
				break;
		}
		delay--;
	}
