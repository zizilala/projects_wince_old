...
static AM3517Bus_t *pAm3517Bus = NULL;
    ...
    )
{
    if (pAm3517Bus == NULL)
	{
        pAm3517Bus = new AM3517Bus_t();
        if (pAm3517Bus->InitializePowerBus() == FALSE)
        {
            delete pAm3517Bus;
            pAm3517Bus = NULL;
		}
    ...
    )
{
    ...
    if (pAm3517Bus != NULL)
    {
        pAm3517Bus->DeinitializePowerBus();
        delete pAm3517Bus;
        pAm3517Bus = NULL;
    }
}
    ...
    )
{
	...
	// Do some device-specific deinit tasks here
    ...
    case IOCTL_BUS_REQUEST_CLOCK: 
		{
			...
			//nmcca: There are other devices that use the bus... Why is this limited to the SGX????? 
			//// Make sure the device ID passed is for SGX
			//if (deviceID != OMAP_DEVICE_SGX)
			//{
			//	ERRORMSG(TRUE, (TEXT("OmapBus::IOCTL_BUS_REQUEST_CLOCK called with device Id \
			//		other than OMAP_DEVICE_SGX\r\n")));
			//	return FALSE;
			//}

			//EnableDeviceClocks(OMAP_DEVICE_SGX, TRUE);
			EnableDeviceClocks(deviceID, TRUE);
    ...
    case IOCTL_BUS_RELEASE_CLOCK: 
		{
			...
			if (deviceID != OMAP_DEVICE_SGX)
			{
				ERRORMSG(TRUE, (TEXT("OmapBus::IOCTL_BUS_RELEASE_CLOCK called with device Id \
					other than OMAP_DEVICE_SGX\r\n")));
				return FALSE;
			}

			EnableDeviceClocks(OMAP_DEVICE_SGX, FALSE);

			return TRUE;
		}
    default:
        return DefaultBusDriver::IOControl(dwCode, pBufIn, dwLenIn, pBufOut, dwLenOut, pdwActualOut);
    }
}
